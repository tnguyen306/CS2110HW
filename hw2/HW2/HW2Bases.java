...
public class HW2Bases
{
	...
	public static int strdtoi(String decimal)
	{
		...
		for ( int i = 0; i < decimal.length(); i++) {
			answer *=10;
			answer += decimal.charAt(i) - '0';
			// That line above converts ASCHII to decimal because subtracting to 0
		}
		return answer;
	}
	...
	public static int strbtoi(String binary)
	{
		...
		for ( int i = 0; i < binary.length(); i++) {
			...
			// again, that line convert ASCHII to decimal
		}
		return answer;
	}
	...
	public static String itostrx(int hex)
	{
		...
		while ( hex >= 1) {
			tracker = hex - ((hex >> 4) <<4); // base 16
			if (tracker > 9) {
				answer = (char) (tracker + 55) + answer;
				// basically find the letter by adding 55
			} else {
				answer = tracker + answer;
			}
			hex = hex >> 4; // divide out each time 
		}
		return answer;
	}
}
